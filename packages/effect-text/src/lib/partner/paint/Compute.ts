    ...
    if (item = getLeafPaint(attrName, paints[i], ui)) {
      ...
      if (item.strokeStyle) {
        ...
        if (item.strokeStyle.strokeWidth)
          maxChildStrokeWidth = Math.max(maxChildStrokeWidth, item.strokeStyle.strokeWidth as number)
      }
    }
  }
...
function getLeafPaint(attrName: string, paint: IPaint, ui: IUI): ILeafPaint {
  if (!isObject(paint) || paint.visible === false || paint.opacity === 0)
    return undefined
  ...
  if (leafPaint) {
    ...
    // 描边样式
    if (isString(leafPaint.style) && hasTransparent(leafPaint.style))
