...
export function fills(fills: ILeafPaint[], ui: IUI, canvas: ILeaferCanvas, renderOptions: IRenderOptions): void {
  ...
  for (let i = 0, len = fills.length; i < len; i++) {
    ...
    if (item.image) {
      ...
      if (PaintImage.checkImage(item, !ui.__.__font, ui, canvas, renderOptions))
        continue

      if (!item.style) {
        if (countImage === 1 && item.image.isPlacehold)
          ui.drawImagePlaceholder(item, canvas, renderOptions) // 图片加载中或加载失败
        continue
      }
    }
    ...
    if (item.transform || originPaint.scaleFixed) {
      ...
      if (originPaint.scaleFixed) {
        ...
        if (originPaint.scaleFixed === true || (originPaint.scaleFixed === 'zoom-in' && scaleX > 1 && scaleY > 1))
          canvas.scale(1 / scaleX, 1 / scaleY)
      }
      if (originPaint.blendMode)
        canvas.blendMode = originPaint.blendMode
      fillPathOrText(ui, canvas, renderOptions)
      canvas.restore()
    }
    else {
