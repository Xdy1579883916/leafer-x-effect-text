import type { ITextData, ITextDecorationType, ITextDrawData } from '@leafer-ui/interface'
...
export function decorationText(drawData: ITextDrawData, style: ITextData): void {
  ...
  if (isObject(textDecoration)) {
    ...
    if (textDecoration.offset)
      offset = Math.min(fontSize * 0.3, Math.max(textDecoration.offset, -fontSize * 0.15))
  }
  else {
    type = textDecoration
  }
