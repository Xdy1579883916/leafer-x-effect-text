import type { ITextCharData, ITextData, ITextDrawData, ITextRowData } from '@leafer-ui/interface'
  ...
  rows.forEach((row) => {
    if (row.words) {
      ...
      if (justify) {
        ...
        if (justifyLetter)
          addLetterWidth = remainingWidth / (row.words.reduce((total, item) => total + item.data.length, 0) - 1) //  remainingWidth / （lettersLength - 1）
